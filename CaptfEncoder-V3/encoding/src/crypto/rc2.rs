    ...
    let val = {
        match cipher_mode.as_str() {
            "CBC" => match padding_mode.as_str() {
                "Pkcs7" => {
                    type Rc2_ = Cbc<Rc2, Pkcs7>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                "AnsiX923" => {
                    type Rc2_ = Cbc<Rc2, AnsiX923>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                "Iso7816" => {
                    type Rc2_ = Cbc<Rc2, Iso7816>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                "NoPadding" => {
                    type Rc2_ = Cbc<Rc2, NoPadding>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                "ZeroPadding" => {
                    type Rc2_ = Cbc<Rc2, ZeroPadding>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                _ => None,
            ...
            "ECB" => match padding_mode.as_str() {
                "Pkcs7" => {
                    type Rc2_ = Ecb<Rc2, Pkcs7>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                "AnsiX923" => {
                    type Rc2_ = Ecb<Rc2, AnsiX923>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                "Iso7816" => {
                    type Rc2_ = Ecb<Rc2, Iso7816>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                "NoPadding" => {
                    type Rc2_ = Ecb<Rc2, NoPadding>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                "ZeroPadding" => {
                    type Rc2_ = Ecb<Rc2, ZeroPadding>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                _ => None,
            ...
            "CFB" => match padding_mode.as_str() {
                "Pkcs7" => {
                    type Rc2_ = Cfb<Rc2, Pkcs7>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                "AnsiX923" => {
                    type Rc2_ = Cfb<Rc2, AnsiX923>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                "Iso7816" => {
                    type Rc2_ = Cfb<Rc2, Iso7816>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                "NoPadding" => {
                    type Rc2_ = Cfb<Rc2, NoPadding>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                "ZeroPadding" => {
                    type Rc2_ = Cfb<Rc2, ZeroPadding>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                _ => None,
            ...
            "OFB" => match padding_mode.as_str() {
                "Pkcs7" => {
                    type Rc2_ = Ofb<Rc2, Pkcs7>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                "AnsiX923" => {
                    type Rc2_ = Ofb<Rc2, AnsiX923>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                "Iso7816" => {
                    type Rc2_ = Ofb<Rc2, Iso7816>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                "NoPadding" => {
                    type Rc2_ = Ofb<Rc2, NoPadding>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                "ZeroPadding" => {
                    type Rc2_ = Ofb<Rc2, ZeroPadding>;
                    let cipher = Rc2_::new_from_slices(&key.as_bytes(), &iv.as_bytes())?;
                    Some(cipher.encrypt_vec(input.as_bytes()))
                }
                _ => None,
