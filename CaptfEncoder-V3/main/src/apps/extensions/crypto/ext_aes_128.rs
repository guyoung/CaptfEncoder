...
impl super::super::IExtensionComponent for Component {
    fn render(&self, grp: &Group) {
        ...
        options.push(ExtensionOption {
            ...
            datatype: "text".to_string(),
            default: "".to_string(),
            tooltip: "ECB mod can be empty,other mod length 16".to_string(),
            items: vec![],
        });
...
impl super::super::IEncoderHandler for Component {
    fn encode(
        ...
        let result = encoding::crypto::aes_128::encode(input, Some(options.clone()));
        match result {
    ...
    fn decode(
        ...
        let result = encoding::crypto::aes_128::decode(input, Some(options.clone()));
        match result {
            Ok(val) => Some(Box::new(val)),
            Err(e) => Some(Box::new(encoding::EncodingResult {
                successed: false,
