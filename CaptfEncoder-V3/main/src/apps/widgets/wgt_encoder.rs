...
impl WgtEncoder {
    pub fn new(grp: &Group, options: &Vec<ExtensionOption>) -> Self {
        ...
        toggle.set_align(Align::Left | Align::Inside);
        toggle.set_frame(FrameType::RFlatBox);
        toggle.set_label("Encode");
        toggle.clear_visible_focus();
        toggle.set_tooltip("Switching encoding and decoding");
        toggle.set_trigger(CallbackTrigger::Changed);
        row.set_size(&toggle, 100);
        row.end();
