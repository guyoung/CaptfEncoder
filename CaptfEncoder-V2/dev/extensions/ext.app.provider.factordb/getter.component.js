...
<v-container fluid class="ma-2">   
    ...
    <v-row height="30" >    
        <v-toolbar flat >
            <v-text-field type="text" v-model="input" outline :label="$t('message.number')">       
            </v-text-field>
            <v-spacer></v-spacer>
    ...
    methods: {
        async get() {
            ...
            if (this.input) {
                ...
                const result = await this.$extInvoke('ext.app.provider.factordb.get', this.input, this.options.value);
