export class AutokeyCipherEncoder {
  _keyword = "";
    ...
    for(let i=0; i<plaintext.length; i++){ 
        if(i < k.length){
            ciphertext += String.fromCharCode(
                (((plaintext.charCodeAt(i)-97) + (k.charCodeAt(i)-97)+26)%26)+97); 
        }else{
            ciphertext += String.fromCharCode(
                (((plaintext.charCodeAt(i)-97) + (plaintext.charCodeAt(i-k.length)-97)+26)%26)+97);
        }    
    } 
