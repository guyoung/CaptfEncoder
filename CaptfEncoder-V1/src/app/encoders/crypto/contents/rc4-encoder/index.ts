...
import { RC4Encoder } from "../../services/RC4Encoder";
import { RC4Decoder } from "../../services/RC4Decoder";

@Component({
  selector: "app-content-rc4-encoder",
  templateUrl: "./index.html",
  styleUrls: ["./index.scss"]
})
export class RC4EncoderComponent implements OnInit, AfterViewInit,  OnDestroy {
  ...
  async handleText() {    
    ...
    try {
      if (this.flagForButtonToggle == "ENCODE") {
        this.txtText2.val = await new RC4Encoder({
        ...
        this.txtText2.val = await new RC4Decoder({
          encoding: this.optEncoding,
          key: this.optKey,       
          output: this.optOutput,
        }).handle(this.txtText1.val);
...
@NgModule({
  ...
  exports: [RC4EncoderComponent],
  declarations: [RC4EncoderComponent],
  entryComponents: [RC4EncoderComponent]
})
export class RC4EncoderModule {
  static entry = RC4EncoderComponent;
}
