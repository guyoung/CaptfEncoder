...
import { AESEncoder } from "../../services/AESEncoder";
import { AESDecoder } from "../../services/AESDecoder";

@Component({
  selector: "app-content-aes-encoder",
  templateUrl: "./index.html",
  styleUrls: ["./index.scss"]
})
export class AESEncoderComponent implements OnInit, AfterViewInit,  OnDestroy {
  ...
  async handleText() {    
    ...
    try {
      if (this.flagForButtonToggle == "ENCODE") {               
        this.txtText2.val = await new AESEncoder({
        ...
        this.txtText2.val = await new AESDecoder({
          encoding: this.optEncoding,
          key: this.optKey,         
          output: this.optOutput,
        }).handle(this.txtText1.val);
...
@NgModule({
  ...
  exports: [AESEncoderComponent],
  declarations: [AESEncoderComponent],
  entryComponents: [AESEncoderComponent]
})
export class AESEncoderModule {
  static entry = AESEncoderComponent;
}
